...
func (d *DionysosHandler) health(w http.ResponseWriter, req *http.Request) {
	...
	if traceCall {
		...
		go d.Config.Tracer.Trace(context.Background(), traceReceived)
	}

	w.Header().Set(plato.HeaderKey, requestId)

	elasticHealth := d.Config.Elastic.Health().Info()
...
func (d *DionysosHandler) checkGrammar(w http.ResponseWriter, req *http.Request) {
	...
	if queryWord == "" {
		...
		if traceCall {
			...
			go d.Config.Tracer.Span(context.Background(), span)
		}
	...
	ttl := time.Hour
	err := d.Config.Cache.SetWithTTL(queryWord, string(stringifiedDeclension), ttl)
	if err != nil {
		e := models.ValidationError{
			ErrorModel: models.ErrorModel{UniqueCode: traceID},
			Messages: []models.ValidationMessages{
