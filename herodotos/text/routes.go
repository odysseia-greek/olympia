...
func InitRoutes(herodotosHandler *HerodotosHandler) *mux.Router {
	...
	serveMux.HandleFunc("/herodotos/v1/options", middleware.Adapt(herodotosHandler.options, middleware.ValidateRestMethod("GET"), middleware.Adapter(comedy.TraceWithLogAndSpan(herodotosHandler.Streamer))))
