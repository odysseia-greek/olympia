...
func (s *SokratesHandler) CloseTrace(response *http.Response, body interface{}) {
	...
	if traceCall {
		...
		s.Tracer.CloseTrace(context.Background(), traceCloser)
...
func (s *SokratesHandler) CloseTraceWithError(err error, requestId string) {
	traceID, parentspanId, traceCall := ParseHeaderID(requestId)
